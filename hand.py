...
# Backspace 제스처 임계값/지연 (초)
BACKSPACE_HOLD_TIME = 0.25
BACKSPACE_REPEAT_TIME = 0.6
PINCH_THRESHOLD_RATIO = 0.18
...
with HandLandmarker.create_from_options(options) as landmarker:
    while True:
        ...
        if detection_result and detection_result.hand_landmarks:
            for idx, hand_landmarks in enumerate(detection_result.hand_landmarks):
                ...
                if hand_type == "Right":
                    ...
                    hand_size = np.hypot(wrist.x - middle_tip.x, wrist.y - middle_tip.y)
                    if hand_size > 0:
                        pinch_ratio = pinch_dist / hand_size
                        if pinch_ratio < PINCH_THRESHOLD_RATIO:
        ...
        # Backspace 제스처 처리 (오른손 pinch)
        if is_active and right_pinch_active:
            if pinch_since is None:
                pinch_since = now
            if (now - pinch_since) >= BACKSPACE_HOLD_TIME and (now - last_backspace_time) >= BACKSPACE_REPEAT_TIME:
                if current_initials:
                    current_initials.pop()
        ...
        if is_active and (not right_pinch_active) and 1 <= total_right_fingers <= len(INITIAL_CONSONANTS):
        ...
        if right_pinch_active:
            img = draw_korean_text(img, "BACKSPACE", (split_x + 10, h - 210), color=(255, 80, 80))
